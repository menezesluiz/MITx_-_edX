...
$10. Why did we make it that way? You can try running
...
more slowly, especially in cases with very large balances and interest
rates. (Note: when your code is running on our servers, there are limits on
the amount of computing time each submission is allowed, so your observations
...
balance - the outstanding balance on the credit card annualInterestRate -
...
fast it is (try the same large inputs in your solution to Problem 2
...
balance_beggin = balance
monthlyInterestRate = annualInterestRate / 12
lower = balance_beggin / 12
upper = (balance_beggin * (1 + monthlyInterestRate) ** 12) / 12
epsilon = 0.03

while abs(balance) > epsilon:
    monthlyPaymentRate = (upper + lower) / 2
    balance = balance_beggin
    for i in range(12):
